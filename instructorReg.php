...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (empty(trim($_POST["username"]))) {
        ...
        $sql = "SELECT id FROM instructorreg WHERE username = ?";
    ...
    if (empty($username_err) && empty($password_err) && empty($confirm_password_err)) {
        $sql = "INSERT INTO instructorreg (full_name, email, username, password, phone_number) VALUES (?, ?, ?, ?, ?)";
        $stmt = mysqli_prepare($conn, $sql);
        if ($stmt) {
...
<body>
    <div class="container mt-5">
        ...
        <p>Already have an account? <a href="instructorlogin.php">Login here</a>.</p>
    </div>
